...
settings = {
    ...
    "man2":"ตั้งข้อความ",
    "man3":"ตั้งข้อความ",
    "message":"บัญชีนี้ได้การป้องกันโดย Ŧ€₳M↭☪➴͜͡✯™ℓສื❍ℓথஆ➴͜͡✯๛ ระบบได้ทำการบล็อคคุณอัตโนมัติ\nติดต่อเชลบอทเม้นในโพสเลยคับ",
...
#==============================================================================#
                ...
                elif text.lower() == 'เมีย':
                    line.sendMessage(to, "🙏อ้าวเตรียมธูปเทียนละเตรียม🙇กราบ🙏")
                    line.sendMessage(to, "รักเมียต้องอดทน ต้องเป็นคนเคารพเมีย 🙇")
                    line.sendMessage(to, "รักเมียต้องส่งเสีย อย่าให้เมียต้องสงสัย 🙇")
                    line.sendMessage(to, "รักเมียต้องรักเดียว อย่าได้เที่ยวไปรักใคร🙇")
                    line.sendMessage(to, "รักเมียต้องทำใจ ถึงอย่างไรเธอก็เมีย🙇")
                    line.sendMessage(to, "รักเมียอย่าอ่อนเพลีย คนรักเมียต้องเข็งแรง 🙇")
                    line.sendMessage(to, "รักเมียอย่าเที่ยวดึก มันจะคึกผิดสำแดง🙇")
                    line.sendMessage(to, "รักเมียอย่ารุนแรง ค่อยๆ แซงอย่าขับไว🙇")
                    line.sendMessage(to, "รักเมียต้องยอมเมีย เพราะว่าเมียไม่ยอมใคร 🙇")
                    line.sendMessage(to, "รักเมียอย่าเถียงเมีย คำพูดเมียใหญ่กว่าใคร🙇")
                    line.sendMessage(to, "ชาติหน้ามีฉันใด จงจำไว้อย่าสู้เมีย🙇")
                elif text.lower() == 'ด่ามัน':
                    line.sendMessage(to, "ห้าวอ่อได้ๆๆ")
                    line.sendMessage(to, "ไอ้สัสนรก ทำห้าว")
                    line.sendMessage(to, "พูดจาเป็นหมาเห่าเจ้า")
                    line.sendMessage(to, "ไรหนักหนา ปากหมาทำห้าว")
                    line.sendMessage(to, "ไอ้แม่เย็ด กากก็เห่าเยอะ")
                    line.sendMessage(to, "รู้จักกูป่าว กูคนให้ข้าวมึงไง๊")
                    line.sendMessage(to, "ร้องลั่นไปดิ หน้าหมี")
                    line.sendMessage(to, "หน้าตาแบบนี้ เหมือนหีเลยว่ะ")
                    line.sendMessage(to, "ล้างหน้าดูบ้าง หมอยขึ้นปะ")
                    line.sendMessage(to, "ควยจะทำไมกับกู อย่าเข้ามาเหม็น")
                    line.sendMessage(to, "ปากดีตีกับพวกกูปะ")
                    line.sendMessage(to, "งัดมารอตีนยุ")
#===============หาชื่อล่อนหน===============================================================#
                elif text.lower() == '!แทค':
                    gs = line.getGroup(to)
                    targets = []
                    ...
                    line.sendMessage(to, "บอท☪➴͜͡✯™ℓສื❍ℓথஆ➴͜͡✯๛")
                    ...
                    start = time.time()
                    line.sendMessage(to,"วัดสปีดโดย..☪➴͜͡✯™ℓສื❍ℓথஆ➴͜͡✯๛")
                    ...
                    line.sendMessage(to, "〘Ŧ€₳M↬☪➴͜͡✯™ℓສื❍ℓথஆ➴͜͡✯๛〙\n""ระยะเวลาการทำงานของบอท {}".format(str(runtime)))
                elif text.lower() == 'บอท':
                    try:
                        arr = []
                        owner = "ub086ba2f15292141da1d900723003fe3"
...
#==============================================================================#
                    ...
                    sendMessageWithMention(to, lineMID)
                    line.sendContact(to, "ub086ba2f15292141da1d900723003fe3")
                elif text.lower() == 'มิด':
...
#==============================================================================#
                    ...
                    for a in range(k+1):
                        ...
                        line.sendMessage(to, "มีคนอาศัย {} คน".format(str(len(nama)))) 
                elif text.lower() == 'แทค':
                    group = line.getGroup(msg.to)
                    nama = [contact.mid for contact in group.members]
                    k = len(nama)//100
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["me","คท","Me",".คท","!me","/me"]:
                    line.sendText(msg.to,"👣ดูควยยยยไร👣")
                if msg.text in ["speed","sp","Speed","สปีด","!speed","/speed"]:
                    line.sendText(msg.to,"👍แรงกว่านี้ก็เอวพี่ละคับ👍")
                if msg.text in ["อย","อยู่"]:
                    line.sendText(msg.to,"❤ชุดต้อนรับทำงาน❤")
                if msg.text in ["แทค",".แทค"]:
                    line.sendText(msg.to,"แทคควยว่างหรอ")				
